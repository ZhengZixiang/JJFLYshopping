...
<title>SimpleSearchHandler</title>
</head>
...
<body>
	...
	<div align="center">
		...
		<% if(pageNo != 1) { %>
		<a href="ProductSearch2.jsp?action=<%= action %>&keyword=<%= keyword %>&categoryid=<%= categoryId %>&pageno=<%= pageNo-1 %>">上一页</a>
		<% } %>
		<% if(pageNo != pageCnt) {%>
		<a href="ProductSearch2.jsp?action=<%= action %>&keyword=<%= keyword %>&categoryid=<%= categoryId %>&pageno=<%= pageNo+1 %>">下一页</a>
		<% } %>
		<a href="ProductSearch2.jsp?action=<%= action %>&keyword=<%= keyword %>&categoryid=<%= categoryId %>&pageno=<%= pageCnt %>">尾页</a>
	</div>
	<center>
